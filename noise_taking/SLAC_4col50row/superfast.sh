...
ac_num_rows=$(mas_param get ac_num_rows)

while IFS=' ' read -r row cs cols; do
    ...
    # mce_freeze_servo_mux11d.py's --row is the physical ac row address
    # (0..ac_num_rows-1), while the channel list's row is the flattened
    # logical row (0..49) across all ac2 chip-select groups -- reduce it
    # to its physical address within its own group before passing it on.
    ac_row=$(( row % ac_num_rows ))

    echo "python $FREEZE_SCRIPT --row $ac_row --ac2-cs $cs --keep-tes-bias sq1"
    python $FREEZE_SCRIPT --row $ac_row --ac2-cs $cs --keep-tes-bias sq1
    if [ $? -ne 0 ]; then
