...
#
# RS - 2026-08-20 master script dispatching to sibling sub-scripts
# RS - 2026-08-23 row_len/sample_dly/filter sweep moved here from
#      normal_dm10.sh/normal_dm1.sh/fast.sh, since all three sweep the same
#      row_len list; superfast.sh keeps its own fixed row_len and is not
#      part of this sweep
# RS - 2026-08-24 moved out of dev/ into BA_I6/ directly
#
...
#                                  "f,s", or "n,f,s". At each bias step, n and f run
#                                  once per row_len in --rowlens (n runs
...
#   -n, --nsamp N                 samples-per-acquisition override, passed through to
#                                  whichever sub-script(s) run (each has its own default
#                                  if omitted; see normal_dm10.sh/normal_dm1.sh/fast.sh/
#                                  superfast.sh usage for their individual defaults)
#
# Examples:
...
#
#   # Normal only (DM10 + DM1), quick smoke test.
#   ./noise_taking_ba_i6.sh --type n --tes-bias-list tes_bias_list_test.txt \
...
#
#   # All three types together, quick smoke test at a single bias.
#   ./noise_taking_ba_i6.sh --type n,f,s --tes-bias-list tes_bias_list_test.txt \
#       --rowlens 119 --row-list fast_row_list_test.txt \
#       --channel-list superfast_channel_list_test.txt --overwrite
#
#   # Multiple row_lens: n and f each run once per row_len listed (superfast
#   # is unaffected, since it uses its own fixed row_len).
#   ./noise_taking_ba_i6.sh --type n,f --tes-bias-list tes_bias_list_test.txt \
#       --rowlens 119,99,79,59 --row-list fast_row_list_test.txt \
#       --nsamp 1000 --overwrite
#
#   # Production run: all three types, full bias sweep, full row_len sweep,
#   # default nsamp for every sub-script.
#   ./noise_taking_ba_i6.sh --type n,f,s --run 1
...
opts=$(getopt -o R:C:b:l:f:r:c:n: \
    --long type:,run:,overwrite,configprefix:,max-cols:,max-rows:,tes-bias-list:,rowlens:,f-cutoff:,row-list:,channel-list:,nsamp: \
    -n "$SCRIPT_NAME" -- "$@")
...
do_n=false; has_type n && do_n=true
...
do
    ...
    echo "bias and settle for 300s"
    bias_tess $bias_args
    sleep 300

    nsamp_arg=()
    [ -n "$nsamp" ] && nsamp_arg=(--nsamp "$nsamp")

    # normal (n) and fast (f) sweep row_len; superfast (s) uses its own
    # fixed internal row_len, so it runs once per bias, outside this loop.
    for rlen in "${rlen_arr[@]}"; do
        if $do_n; then
            reconfig_for_rowlen $rlen
            "$SCRIPT_DIR/normal_dm10.sh" --dir "$biasdir" --rowlen "$rlen" "${nsamp_arg[@]}"

            reconfig_for_rowlen $rlen
            "$SCRIPT_DIR/normal_dm1.sh" --dir "$biasdir" --rowlen "$rlen" "${nsamp_arg[@]}"
        fi

        if $do_f; then
            reconfig_for_rowlen $rlen
            "$SCRIPT_DIR/fast.sh" --dir "$biasdir" --rowlen "$rlen" --row-list "$row_list" "${nsamp_arg[@]}"
        fi
    done
    ...
    if $do_s; then
        ...
        mce_reconfig
        sleep 1
        "$SCRIPT_DIR/superfast.sh" --dir "$biasdir" --channel-list "$channel_list" "${nsamp_arg[@]}"
    fi
done
